...
pub trait Functor<A: 'static> {
	type Map<B: 'static>: Functor<B>;
	...
	fn map<B, F: 'static + FnMut(A) -> B>(self, f: F) -> Self::Map<B>;
}

/// A `Functor` with application, providing operations to
/// * embed "pure" expressions, and
/// * sequence computations and combine their results.
pub trait Applicative<A: 'static>: Functor<A> {
	type Apply<B: 'static>: Applicative<B>;

	/// Embeds a value into the structure.
	fn pure(a: A) -> Self;

	/// Sequences computations and combines their results.
	fn ap<B, F: 'static + FnMut(A) -> B>(self, f: Self::Apply<F>) -> Self::Apply<B>;
}

/// A `Applicative` with a monadic binding operation. Representing the idea of
/// a monad from mathematics category theory.
pub trait Monad<A: 'static>: Applicative<A> {
	type Bind<B: 'static>: Monad<B>;

	/// Sequentially compose two actions, passing any value produced by the
	/// first as an argument to the second.
	fn bind<B, F: 'static + FnMut(A) -> Self::Bind<B>>(self, f: F) -> Self::Bind<B>;
}

/// Embeds a "pure" expression `a` into a `Monad`.
pub fn ret<A: 'static, M: Monad<A>>(a: A) -> M {
	M::pure(a)
}
